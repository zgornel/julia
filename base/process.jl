...
function setup_stdio(stdio::Union{IOBuffer, BufferStream}, child_readable::Bool)
    ...
    rd, wr = link_pipe(!child_readable, child_readable)
    try
        open_pipe!(parent, child_readable ? wr : rd, !child_readable, child_readable)
    catch ex
        close_pipe_sync(rd)
        close_pipe_sync(wr)
        rethrow(ex)
...
Start running `command` asynchronously, and return `process::IO`. If `read` is
...
the process's standard input and `other` optionally specifies the process's standard output stream.
The process's standard error stream is connected to the current `stderr`.
"""
